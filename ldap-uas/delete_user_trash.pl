...
sub do_action {
    ...
    if ( $_GET{'confirm'} eq '1' ) {
        my $trash  = ldap_init_config('trash');
        ...
        my $mesg = $ldap->moddn(
            'uid=' . $_GET{'uid'} . ',' . $config->{'suspended_user_dn'},
            newrdn => "uid=$_GET{'uid'}"
                . "\@$config->{'realm_short'}" . "_"
                . time(),
            newsuperior  => $trash->{'user_dn'},
            deleteoldrdn => 1,
        );

        if ( !$mesg->code() ) {
            $log_msg = sprintf( "OK: Delete user [uid = %s]", $_GET{'uid'} );
            $result .= $log_msg . "\n";
        }
        else {
            $log_msg = sprintf( "FAIL: Delete user [uid = %s], [error = %s]",
                $_GET{'uid'}, $mesg->error() );
            $result .= $log_msg . "\n";
        }
...
sub main {
    ...
    my $template
        = HTML::Template->new( filename => getTemplate("delete_user.tmpl") );
