...
sub usage {
    print <<EOF
Usage:
\t$0 [PROFILE] [STOP] [FILTER]    [ANNOUNCEMENT]
...
sub main {
    ...
    foreach my $u ( @{$users} ) {
        ...
        if ( $_VAR{'STOP'} ) {
            print stop_password( $ldap, $config, $u, 1 );
        }
        else {
            print stop_password( $ldap, $config, $u, 0 );
        }
    }
...
sub stop_password {
    ...
    if ($stop) {
        ...
        my $description
            ...
            ? $user->{'description'}
            : '';
        $description .= sprintf( 'adm-stoppasswd(%s,-,%s);',
