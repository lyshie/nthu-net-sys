...
sub last_modified {
    ...
    $mesg = $ldap->search(
        ...
        attrs => [ 'uid', 'cn', 'sn', 'modifyTimestamp' ],
    );

    foreach my $entry ( $mesg->entries() ) {
        my $modified_time
            = defined( $entry->get_value('modifyTimestamp') )
            ? $entry->get_value('modifyTimestamp')
            : '';
        $modified_time = ldap_to_unix_time($modified_time);
        ...
        $result .= sprintf(
            "%-18s [%s] [sn=%s] [cn=%s]\n",
            $uid, scalar( localtime($modified_time) ),
            $sn, $cn
        );
    }
