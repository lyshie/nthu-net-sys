...
sub do_action {
    ...
    if ( $_GET{'confirm'} eq '1' ) {
        ...
        $mesg = $ldap->moddn(
            'uid=' . $_GET{'uid'} . ',' . $config->{'user_dn'},
            newrdn      => 'uid=' . $_GET{'uid'},
            newsuperior => $config->{'suspended_user_dn'},
        );

        if ( !$mesg->code() ) {
            $log_msg = sprintf( "OK: Suspend user [uid = %s]", $_GET{'uid'} );
            $result .= $log_msg . "\n";
        }
        else {
            $log_msg = sprintf( "FAIL: Suspend user [uid = %s], [error = %s]",
                $_GET{'uid'}, $mesg->error() );
            $result .= $log_msg . "\n";
        }
...
sub main {
    ...
    my $template
        = HTML::Template->new( filename => getTemplate("suspend_user.tmpl") );
